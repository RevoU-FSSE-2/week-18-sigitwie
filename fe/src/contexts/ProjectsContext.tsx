...
interface ProjectsContextProps {
  ...
  editProject: (projectId: string, updatedData: any) => void;
...
const useProjects = () => {
  ...
  const addProject = async (projectData: any) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          responseData.message || "The network did not respond appropriately."
        );
      }
  ...
  const editProject = async (projectId: string, updatedData: any) => {
    try {
      const response = await fetch(
        `https://w18.eswe.dev/v1/api/projects/update/${projectId}`,
        {
          method: "PUT",
          headers: {
  ...
  const deleteProject = async (projectId: string) => {
    try {
      const response = await fetch(
        `https://w18.eswe.dev/v1/api/projects/delete/${projectId}`,
        {
          method: "DELETE",
          credentials: "include",
        }
...
const ProjectsProvider: React.FC<ProjectsProviderProps> = ({ children }) => {
  const projectsStateAndMethods = useProjects();

  return (
    <ProjectsContext.Provider value={projectsStateAndMethods}>
      {children}
...
function useProjectData(): ProjectsContextProps {
  const context = useContext(ProjectsContext);
  if (context === undefined) {
    throw new Error("useProjectData must be used within a ProjectsProvider");
  }
  return context;
}

export { ProjectsContext, ProjectsProvider, useProjectData };
